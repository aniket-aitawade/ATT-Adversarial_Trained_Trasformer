...
class ATT_generator(tf.keras.Model):
    def __init__(self,n_fft):
        super().__init__()
        self.n_fft=n_fft
...
class ATT_discriminator(tf.keras.Model):
    def __init__(self):
        super().__init__()
...
class ATT(tf.keras.Model):
    def __init__(self,n_fft, *args, **kwargs):
        ...
        self.generator=ATT_generator(n_fft)
        self.discriminator=ATT_discriminator()
    ...
    def train_step(self, batch):
        noisy,clean=batch
        noisy=tf.reshape(noisy,shape=[-1,tf.shape(noisy)[2],tf.shape(noisy)[3]])
        clean=tf.reshape(clean,shape=[-1,tf.shape(clean)[2],tf.shape(clean)[3]])
        real=clean
    ...
    def predict_step(self, noisy):
        dim0=tf.shape(noisy)[0]
        dim1=tf.shape(noisy)[1]
        dim2=tf.shape(noisy)[2]
        dim3=tf.shape(noisy)[3]
        noisy=tf.reshape(noisy,shape=[dim0*dim1,dim2,dim3])        
        fake=self.generator(noisy,training=False) 
        fake=tf.reshape(fake,shape=[dim0,dim1,dim2,dim3])
        return fake
